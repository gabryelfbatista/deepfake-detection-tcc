...
class SquarePad:
    """Pad a PIL image to a square (the longer side), centered.

    The SID_Set images are not all square, so resizing straight to
    (s, s) would distort the aspect ratio and could erase or warp the
    inpainting/generation artifacts the model relies on. Padding to a
    square first lets the following Resize scale uniformly (no distortion).
    """
...
def build_transforms(cfg, train=True):
    ...
    base = [SquarePad(fill=0), transforms.Resize((img_size, img_size))]
