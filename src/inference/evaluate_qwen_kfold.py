"""
Standalone K-Fold evaluation for experiment_03 (Qwen3-VL-2B + QLoRA).
...
Run on a single GPU — no DDP, no NCCL barriers.
    ...
    uv run python src/inference/evaluate_qwen_kfold.py
    uv run python src/inference/evaluate_qwen_kfold.py --folds 1 2   # specific folds
    uv run python src/inference/evaluate_qwen_kfold.py --adapter-dir experiments/qwen_kfold_03
"""
import os
import sys
import argparse
